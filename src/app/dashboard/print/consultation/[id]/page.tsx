...
import { PrintTrigger } from "@/components/dashboard/print/PrintTrigger"; // We will create this small client component
        ...
        <div className="bg-slate-100 dark:bg-slate-950 min-h-screen py-8 print:bg-white print:p-0 transition-colors duration-500">
            ...
            <header className="border-b-4 border-primary pb-6 mb-8 flex justify-between items-start">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black uppercase tracking-tighter text-slate-900 leading-none">
                            {data.clinic?.name || "Clínica Veterinaria"}
                        </h1>
                        <div className="text-xs text-slate-500 mt-2 space-y-0.5 font-medium">
                            <p>{data.clinic?.address || "Dirección no registrada"}</p>
                            <p>Tel: {data.clinic?.phone}</p>
                        </div>
                    </div>
                </div>
                <div className="text-right">
                    ...
                    <div className="mt-4 text-[10px] font-bold text-slate-400 space-y-1">
                        <p className="uppercase tracking-widest">Folio: #{data.id.slice(0, 8)}</p>
                        <p className="text-slate-900 text-lg">
                            {new Date(data.visit_date).toLocaleDateString("es-ES", { day: 'numeric', month: 'long', year: 'numeric' })}
                        </p>
                    </div>
                </div>
            </header>
            ...
            <section className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6 text-sm grid grid-cols-2 gap-y-2">
                <div>
                    ...
                    <span className="text-lg font-bold">{data.patient?.name}</span>
                    <span className="text-gray-600 ml-2">({data.patient?.species} - {data.patient?.breed})</span>
                </div>
                <div>
                    <span className="font-bold text-gray-500 uppercase text-xs block">Propietario</span>
                    <span className="text-lg font-medium">{data.patient?.owner?.full_name}</span>
                </div>
                <div>
                    <span className="font-bold text-gray-500 uppercase text-xs block">Edad / Sexo</span>
                    <span>{getAge(data.patient?.birth_date)} • {data.patient?.sex === 'male' ? 'Macho' : 'Hembra'}</span>
                </div>
                <div>
                    <span className="font-bold text-gray-500 uppercase text-xs block">Vitals (Peso / Temp)</span>
                    <span>
            ...
            <div className="space-y-6">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-lg font-medium text-gray-800">{data.assessment || "Consulta General"}</p>
                </div>
                ...
                <div className="break-inside-avoid">
                    <h3 className="font-bold border-b border-gray-300 pb-1 mb-4 uppercase text-sm flex justify-between">
                        ...
                        <span className="text-xs font-normal text-gray-400">Dr(a). {data.vet?.full_name}</span>
                    </h3>
                    <div className="whitespace-pre-wrap text-base leading-relaxed font-medium text-gray-900 font-serif">
                        {data.plan || "Sin indicaciones específicas."}
                    </div>
                </div>
                ...
                {data.subjective && (
                    <div className="mt-8 pt-4 border-t border-gray-100">
                        ...
                        <p className="text-sm text-gray-500 italic">{data.subjective}</p>
                    </div>
                )}
                ...
                <footer className="mt-12">
                    <div className="flex justify-between items-end">
                        ...
                        <div className="text-center">
                            ...
                            <p className="font-bold text-sm">Dr(a). {data.vet?.full_name}</p>
